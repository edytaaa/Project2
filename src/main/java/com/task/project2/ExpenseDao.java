...
public class ExpenseDao{
    ...
    public ArrayList<Expense> readBetweenDates(Date dateFrom, Date dateTo){
        String sql = "select * from home_budget where date >= ? and date <= ?";
        try{
            PreparedStatement prepStmt = connection.prepareStatement(sql);
            prepStmt.setDate(1, dateFrom);
            prepStmt.setDate(2, dateTo);
            return readAll(prepStmt);
        }catch (SQLException e){
            System.out.println("Could not get data");
    ...
    public ArrayList<Expense> readAboveAmount(double amountAbove){
        String sql = "select * from home_budget where amount > ?";
        try{
            PreparedStatement prepStmt = connection.prepareStatement(sql);
            prepStmt.setDouble(1, amountAbove);
            return readAll(prepStmt);
        }catch (SQLException e){
            System.out.println("Could not get data");
